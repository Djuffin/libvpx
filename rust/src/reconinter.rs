...
// ===========================================================================
// Encoder-only chroma helpers
// ===========================================================================

/// `vp8_build_inter16x16_predictors_mbuv` — encoder-only chroma
/// predict into `MACROBLOCKD::predictor` scratch slots [256..384].
///
/// Source: `vp8/common/reconinter.c:136`.
pub unsafe fn vp8_build_inter16x16_predictors_mbuv(x: *mut Macroblockd) {
    let uptr: *mut u8;
    let vptr: *mut u8;
    let upred_ptr: *mut u8 = (*x).predictor.as_mut_ptr().offset(256);
    let vpred_ptr: *mut u8 = (*x).predictor.as_mut_ptr().offset(320);

    let mb_mv = (*(*x).mode_info_context).mbmi.mv;
    let mut mv_row: i32 = mb_mv.row as i32;
    let mut mv_col: i32 = mb_mv.col as i32;
    let offset: i32;
    let pre_stride: i32 = (*x).pre.uv_stride;

    /* calc uv motion vectors */
    mv_row += 1 | (mv_row >> (core::mem::size_of::<i32>() as i32 * 8 - 1));
    mv_col += 1 | (mv_col >> (core::mem::size_of::<i32>() as i32 * 8 - 1));
    mv_row /= 2;
    mv_col /= 2;
...
/// `vp8_build_inter4x4_predictors_mbuv` — encoder-only SPLITMV chroma
/// predict into per-`BLOCKD` `predictor` scratch.
///
/// Source: `vp8/common/reconinter.c:170`.
pub unsafe fn vp8_build_inter4x4_predictors_mbuv(x: *mut Macroblockd) {
    let mut i: i32;
    let mut j: i32;
    let pre_stride: i32 = (*x).pre.uv_stride;
    ...
    while i < 2 {
        ...
        while j < 2 {
            ...
            temp = bmi_mv(&(*x).block[yoffset as usize].bmi).row as i32
                + bmi_mv(&(*x).block[(yoffset + 1) as usize].bmi).row as i32
                + bmi_mv(&(*x).block[(yoffset + 4) as usize].bmi).row as i32
                + bmi_mv(&(*x).block[(yoffset + 5) as usize].bmi).row as i32;

            temp += 4 + ((temp >> (core::mem::size_of::<i32>() as i32 * 8 - 1)) * 8);

            let new_row = ((temp / 8) & (*x).fullpixel_mask) as i16;

            temp = bmi_mv(&(*x).block[yoffset as usize].bmi).col as i32
                + bmi_mv(&(*x).block[(yoffset + 1) as usize].bmi).col as i32
                + bmi_mv(&(*x).block[(yoffset + 4) as usize].bmi).col as i32
                + bmi_mv(&(*x).block[(yoffset + 5) as usize].bmi).col as i32;
            ...
            // voffset.bmi.as_int = uoffset.bmi.as_int
...
/// `vp8_build_inter16x16_predictors_mby` — encoder-only luma-only 16×16
/// predict, no MV clamp.
///
/// Source: `vp8/common/reconinter.c:238`.
pub unsafe fn vp8_build_inter16x16_predictors_mby(
    ...
    let ptr: *mut u8;
    let mb_mv = (*(*x).mode_info_context).mbmi.mv;
    let mv_row: i32 = mb_mv.row as i32;
    let mv_col: i32 = mb_mv.col as i32;
    let pre_stride: i32 = (*x).pre.y_stride;

    ptr_base = (*x).pre.y_buffer;
    ptr = ptr_base
        .offset(((mv_row >> 3) * pre_stride) as isize)
        .offset((mv_col >> 3) as isize);
