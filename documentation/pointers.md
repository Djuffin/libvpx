...
**`Vp8Common` is fully free of raw-pointer fields** (every slab — `yv12_fb`, `above_context`, `mip` — is owned through safe Rust types) and **`Macroblockd` has only one remaining raw-pointer entry: the `recon_above` / `recon_left` per-plane edge arrays.**
...
### §2 — empirical findings after the MI-grid conversion
...
- **There is a measurable perf cost.** The MI-grid conversion alone added ~1.6-1.7% steady-state regression — `Option::expect` + slice bounds checks where the C source used cursor arithmetic. This is the price of safe indexed access, paid honestly (no `get_unchecked` shortcuts).
...
Making functions like `decode_macroblock` *fully* safe (no `unsafe {}` blocks inside, no raw pointer parameters) remains gated on the `recon_above`/`recon_left` and the embedded `pre`/`dst` YV12 plane pointers — pixel-side state that this document deliberately excludes from its tracking.
