...
**Resolved: loop-filter row filters.** `vp8_loop_filter_row_normal` / `vp8_loop_filter_row_simple` had `cm: *mut Vp8Common` → **`cm: &Vp8Common`** (a *shared* ref — `cm` is read-only: `frame_type`, `mb_cols`, `&lf_info`, `mi_row(mb_row)`). They stay `unsafe fn` for the `y/u/v_ptr: *mut u8` plane pointers + the raw edge-filter kernel calls, but the god-object pointer is gone. The four call sites in `decode_mb_rows` pass `&pbi.common` (shared, disjoint from the local `lf_dst`/`eb_dst` plane pointers). 125/125 conformance; all suites pass.

**The §1 god-object surface is now empty.** No kernel function anywhere takes a `*mut`/`*const` of `Vp8dComp` / `Vp8Common` / `Macroblockd` / `ModeInfo` / `MbModeInfo` / `Blockd` / `FrameBuffers` — every one on the decode / control / setup / loop-filter paths is a reference or removed. What remains:
- **The iface bridge `FrameBuffers::pbi_ptr() -> *mut Vp8dComp`** — used at 5 sites in `vp8_dx_iface.rs` to obtain a raw `pbi` *local* for field access inside the C-ABI iface functions, where `ctx: *mut Vp8AlgPriv` is itself an inherently-raw vtable `void*`. This is the FFI-boundary layer, not the kernel-function-arg category §1 tracks; the read-only control handlers (`GetLastRefUpdates`/`GetFrameCorrupted`/`GetLastRefUsed`) could tidy to `pbi.as_deref()` but it is iface-internal cleanup constrained by the raw `ctx`.
- **The doc-excluded data category**: pixel-plane `*mut u8` (`Yv12BufferConfig`, predictor/IDCT/loop-filter kernels), coeff `*mut i16`, bitstream-input `*const u8` (the bool reader + the two header helpers above), threshold-table `*const u8` (`LoopFilterInfo`), and the threading/mem/FFI shims.
